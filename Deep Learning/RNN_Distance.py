...
Original file is located at
    https://colab.research.google.com/drive/1qzWghhJnPH36gaQ7dcyaw7J-coEkOJc8
"""
...
# Import datasets & load datasets
X_data = list()
Y_data = list()
X_test = list()
Y_test = list()

path = './Labeled Data/Distance/'
...
def get(n_datasets, distance, result, dx, dy):
  filename = input('Please type the name of file : ')
  for i in range(1, n_datasets):
      name = path + filename + '/' + distance + '/' + filename + str(i) + '.csv'
...
def load(x, y, result):
  ...
  get(n_datasets, distance, result, x, y)
...
for i in range(2):
  load(X_data, Y_data, [1, 0, 0])  # Far
  load(X_data, Y_data, [0, 1, 0])  # Close
...
log_path='./Logs/RNN_Distance_20'
save_path = './Model/RNN_Distance_20/RNN'
...
with tf.name_scope('cost'):
  # Cross-Entropy_v2
  Y_pred = tf.contrib.layers.fully_connected(model, 3, activation_fn=tf.nn.relu)
  cost = tf.losses.mean_squared_error(Y, Y_pred)
  cost_summ = tf.summary.scalar('cost', cost)

optimizer = tf.train.AdamOptimizer(learning_rate, name="op_to_restore").minimize(cost)
