...
def get(n_datasets, tag, result, dx, dy):
    for i in range(1, n_datasets):
        name = path + tag + '/' + str(i) + '.csv'
        file = pd.read_csv(name)
        dataset = file.values.tolist()
        split(dataset, result, dx, dy)

def load(x, y, result):
  if str(result) == '[1, 0, 0, 1, 1, 0, 0, 1]':
      tag = '_close_right_walk'
  elif str(result) == '[1, 0, 0, 1, 1, 0, 1, 0]':
      tag = '_close_right_run'
  elif str(result) == '[1, 0, 0, 1, 0, 1, 0, 1]':
      tag = '_close_left_walk'
  elif str(result) == '[1, 0, 0, 1, 0, 1, 1, 0]':
      tag = '_close_left_run'
  elif str(result) == '[1, 0, 1, 0, 1, 0, 0, 1]':
      tag = '_far_right_walk'
  elif str(result) == '[1, 0, 1, 0, 1, 0, 1, 0]':
      tag = '_far_right_run'
  elif str(result) == '[1, 0, 1, 0, 0, 1, 0, 1]':
      tag = '_far_left_walk'
  elif str(result) == '[1, 0, 1, 0, 0, 1, 1, 0]':
      tag = '_far_left_run'
  n_datasets = int(input('Please type the number of datasets : '))
  get(n_datasets, tag, result, x, y)
...
load(X_data, Y_data, [1, 0, 0, 1, 1, 0, 0, 1])
load(X_data, Y_data, [1, 0, 0, 1, 1, 0, 1, 0])
load(X_data, Y_data, [1, 0, 0, 1, 0, 1, 0, 1])
load(X_data, Y_data, [1, 0, 0, 1, 0, 1, 1, 0])
load(X_data, Y_data, [1, 0, 1, 0, 1, 0, 0, 1])
load(X_data, Y_data, [1, 0, 1, 0, 1, 0, 1, 0])
load(X_data, Y_data, [1, 0, 1, 0, 0, 1, 0, 1])
load(X_data, Y_data, [1, 0, 1, 0, 0, 1, 1, 0])
load2(X_data, Y_data, [0, 1, 0, 0, 0, 0, 0, 0])
...
log_path='./Logs/RNN_All_2/'
save_path = './Model/RNN_ALL_2/RNN'
...
with tf.name_scope('cost'):
  Y_pred = tf.contrib.layers.fully_connected(model, 8, activation_fn=tf.nn.relu)
